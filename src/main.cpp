...
// Checks if the SocketID event has JSON data
...
int main() {
    ...
    // Load up map values from waypoint's x, y, s and d normalized vectors
    ...
    // The max s value before wrapping around the track back to zero
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    // car is or at the previous path and points
                    double ref_x = car_x;
                    double ref_y = car_y;
                    double ref_yaw = deg2rad(car_yaw);

                    // If previous size is almost empty, use the car as
                    // steering reference
                    if (prev_size < 2) {
                        // Use two points that make the path
                        // tangent to the car
                    ...
                    // Define the actual (x, y) points we will
                    // use for the planner
